...
form.addEventListener('submit', (e) => {
	e.preventDefault();
	// type of value returned is string --> use parseInt to convert
	const value = parseInt(amount.value);
	// console.log(typeof value);

	const random = Math.floor(Math.random() * text.length);

	// currently there are 9 paragraphs in the text array
	// if user enters a value that's NaN or less than 0 or great than 9, return a random paragraph
	if (isNaN(value) || value <= 0 || value > 9) {
		result.innerHTML = `<p class="result">${text[random]}</p>`;
	} else {
		// slice(first paragraph, to number of paragraphs entered)
